...
async def transcribe_anchor(
    ...
    rejected_ko = None
    # Overlap the ko-guard with the first guard call: the Hangul branch consumes
    # both and neither depends on the other. (A speculative force-KO decode here
    # was measured 2026-09-08 and REVERTED: it cut escalated-tier latency ~20%
    # but fired on every Latin clip, and the ~5x wasted decodes congested the
    # backend enough to cost -30% overall throughput under stress load.)
    g_task = asyncio.ensure_future(guard(audio, filename, en_text=text))
    ko_guard_task = (asyncio.ensure_future(guard(audio, filename, ko_text=text))
                     if HANGUL.search(text) else None)
    if ko_guard_task is not None:
        ko_guard_task.add_done_callback(lambda t: t.cancelled() or t.exception())
    g = await g_task
    ...
    if ko_guard_task is not None and not ko_guard_task.done():
        ko_guard_task.cancel()
