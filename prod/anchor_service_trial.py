...
FOLDED_ONLY = os.getenv("ANCHOR_FOLDED_ONLY", "0") == "1"  # drop the weighted trigger
FOLDED_UNIVERSAL = os.getenv("ANCHOR_UNIVERSAL_FLOOR", "0") == "1"  # apply KO support floor to ALL word counts
XEUS_FALLBACK = os.getenv("ANCHOR_XEUS_FALLBACK", "0") == "1"  # re-score KO with Xeus in the ambiguous ZIPA band
XEUS_SUPPORT = float(os.getenv("ANCHOR_XEUS_SUPPORT", "0.70"))
XEUS_ARBITRATE = os.getenv("ANCHOR_XEUS_ARBITRATE", "0") == "1"  # Xeus scores BOTH EN+KO, argmin + reject if both high
XEUS_REJECT = float(os.getenv("ANCHOR_XEUS_REJECT", "0.8"))
