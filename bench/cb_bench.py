...
URL = 'http://192.168.1.225:8021/v1/chat/completions'
LEVELS = [4, 8, 16]

rows = json.load(open(f'{ROOT}/dataset/stress_test_500.json'))
clips = []
for s in rows:
    p = f"{ROOT}/dataset/stress_clips/{s['audio_url'].split('/')[-1]}"
    if os.path.exists(p):
...
json.dump(out, open(f'{ROOT}/dataset/cb_results.json', 'w'), ensure_ascii=False)
